        ...
        <div class="card-header pb-0">
            <div class="row">
                ...
                <div class="text-end col-lg-3">
                    <button href="#" class="btn btn-md bg-gradient-success" data-bs-toggle="modal" data-bs-target="#ModalInventory"><i class="fa fa-solid fa-plus"></i> Add Materials</button>
                    @include('pages.inventory.addMaterials')
                </div>
                <div class="text-end col-lg-3">
    ...
    document.addEventListener("DOMContentLoaded", function(){
        ...
        if(tableBody && paginationContainer && entriesInfo){
            ...
            const fetchData = (page = 1, search = '') => {
                ...
                axios.get(`{{ route('invDataMaterials') }}?page=${page}&search=${search}`)
                        ...
                        inventoryData.forEach(inventory => {
                            const row = `
                                <tr>
                                    ...
                                    @include('pages.inventory.editMaterials')
            ...
            addIngredientsForm.addEventListener('submit', async function(event) {
                ...
                try{
                    ...
                    if (checkResponse.data.exists) {
                        Swal.fire({
                            icon: 'error',
                            title: 'Materials already exists',
                            text: 'Please choose a different materials name.',
                            showConfirmButton: false,
                            timer: 3000
                        });
                    }else{
                        ...
                        try {
                            var storeResponse = await axios.post('{{ route('invenstorematerials') }}', formData);
